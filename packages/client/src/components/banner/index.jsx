...
function Banner(props) {
  ...
  let isScreenSmall = (screenType === 'mobile');

  return (
    <div className="flex items-center bg-accent py-3 pl-3 pr-4 justify-between">

        <div className="flex items-center w-11/12">
          {!isScreenSmall && props.icon && <SVGIcon icon={props.icon} fill="var(--color-primary-100)" className="w-5 h-5" />}

          <p className={`text-xs sm:text-sm text-primary-100 ml-2 w-6/12 sm:w-8/12 lg:w-10/12 ${props.truncate && "truncate"}`}>
            {props.children}
            {!props.truncate && props.link && <a href={props.link} className="text-xs sm:text-sm text-primary-100 underline ml-1">{props.linkText}</a>}
          </p>
